...
What's trending right now, useful as *examples* of the moves in action: glassmorphism matured into a functional layer — volumetric glass now reserved for secondary surfaces (modals, tooltips, panels) rather than a global theme; AI-copilot side panels and inline assistant chips docked beside content rather than replacing it; a neo-brutalist/anti-design backlash against AI-smooth output — deliberately messy typography, clashing color, off-kilter asymmetry as a signal a human made it; claymorphic soft-3D onboarding (pastel, chunky, tactile) for fintech and consumer apps; bold sculptural 3D hero objects — liquid-metal blobs, warped ribbons — reacting to scroll and hover; oversized fintech balance numbers; scattered/modular grid layouts breaking the rigid 12-column template; small, intentional audio micro-cues joining motion as state feedback; spring-physics micro-interactions. Treat all of it as dated-by-default styling layered over the five moves.
