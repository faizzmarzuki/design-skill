...
What's trending right now (items marked 👁 were verified by *watching* 2026 design videos frame-by-frame, not just reading about them), useful as *examples* of the moves in action: 👁 **maximalist / brutalist brand sites** — a single saturated color field (e.g. all-red), oversized condensed display type set huge, marquee/ticker strips, and technical monospace annotations (arrows, axis labels) as overlay texture; 👁 **editorial minimalism** at the other pole — big serif+sans display type carrying the whole layout, black-on-white, hairline rules between sections, wide-letter-spaced micro-labels; 👁 **sculptural 3D hero objects** (rendered coins, liquid-metal blobs, warped ribbons); Apple-style liquid-glass translucency and frosted depth layers; AI-copilot side panels and inline assistant chips docked beside content rather than replacing it; sci-fi/spaceship-dashboard neon grids on dark UI; atmospheric soft-glow, smoky-gradient hero art (replacing flat aurora-mesh); oversized fintech balance numbers; cream/pastel friendly onboarding; scattered/modular grid layouts breaking the rigid 12-column template; cinematic hero video backgrounds; spring-physics micro-interactions. Treat all of it as dated-by-default styling layered over the five moves.
...
Learned by studying ~40 currently-trending UI/design posts on X (July 2026), synthesized by Opus 4.8 with Fable 5 as a taste advisor, then extended with a **local video-watch pass** — watching current design-trend videos on YouTube frame-by-frame (real vision, including motion) rather than reading text digests, since seeing is the strongest input to design taste. The costumes will age; the five moves and the guardrails are the durable part — refresh the "current register" section periodically, leave the rest.
